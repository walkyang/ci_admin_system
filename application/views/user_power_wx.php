                        ...
                        <div class="am-btn-toolbar am-fl">
                            ...
                            <div class="am-form-group am-margin-left ">
                                <select id="model_id" name="model_id" multiple="multiple" style="width: 20%">
                                    <option value="10">新房</option>
                                    <option value="20">二手</option>
                                </select>
                            </div>
                            <div class="am-form-group am-margin-left am-fl">
                        ...
                        <div class="am-btn-toolbar am-fl">
                            <div class="am-form-group am-margin-left am-fl">
                                ...
                                <?php foreach($user_city_power as $k=>$v){?>
                                    <a href="/user/user_power_wx/?id=<?=$user_id;?>&city_id=<?=$v->city_id;?>"><?=$v->city_name;?>(<?php echo $v->max_date > date('Y-m-d') ? $v->max_date: $v->max_date.'[过期]'?>)</a>&nbsp;&nbsp;
                                <?php }?>
                            </div>
                        </div>
                    </div>
...
<script>
    $('#city_id').change(function(){
        var id = $('#user_id').val();
        window.location.href='/user/user_power_wx/?id='+id+'&city_id='+$(this).val();
    });
    $('.add_power').click(function(){
        ...
        $.post("/user/add_power", {
            ...
            if(data == -1){
                ...
                alert('添加成功');
                window.location.href='/user/user_power_wx/?id='+$('#user_id').val()+'&city_id='+$('#city_id').val();
            }else{
                alert('出错了，请联系管理');
                return;
            }
